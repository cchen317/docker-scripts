...
function start_master() {
    ...
    if [ "$DEBUG" -gt 0 ]; then
        echo sudo docker run -d -P --dns $NAMESERVER_IP -h master${DOMAINNAME} $VOLUME_MAP $1:$2
    fi
    MASTER=$(sudo docker run -d -P --dns $NAMESERVER_IP -h master${DOMAINNAME} $VOLUME_MAP $1:$2)
...
function start_workers() {
	...
	hostname="worker${i}${DOMAINNAME}"
        if [ "$DEBUG" -gt 0 ]; then
	    echo sudo docker run -d -P --dns $NAMESERVER_IP -h $hostname $VOLUME_MAP $1:$2 ${MASTER_IP}
        fi
	WORKER=$(sudo docker run -d -P --dns $NAMESERVER_IP -h $hostname $VOLUME_MAP $1:$2 ${MASTER_IP})
