...
function start_nameserver() {
    ...
    if [ "$DEBUG" -gt 0 ]; then
        echo sudo docker run -d -P -h nameserver${DOMAINNAME} -v $DNSDIR:/etc/dnsmasq.d $1
    fi
    NAMESERVER=$(sudo docker run -d -P -h nameserver${DOMAINNAME} -v $DNSDIR:/etc/dnsmasq.d $1)
