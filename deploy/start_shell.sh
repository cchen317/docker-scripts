...
function start_shell() {
    ...
    if [ "$DEBUG" -gt 0 ]; then
        echo sudo docker run -i -t -d -P --dns $NAMESERVER_IP -h $SHELL_HOSTNAME $VOLUME_MAP $IMAGENAME $MASTER_IP
    fi
    SHELL_ID=$(sudo docker run -i -t -d -P --dns $NAMESERVER_IP -h $SHELL_HOSTNAME $VOLUME_MAP $IMAGENAME $MASTER_IP)
